...
export default function EditCategProd() {
  ...
  const changeOnClick = async (e) => {
    ...
    console.log(categorybios)
    try {
      const res = await axios.put('http://localhost:3002/api/maincategbio/'+ editcatgId , categorybios)
      if(res.status === 200) {
        alert('updated')
      }
    } catch (error) {
  ...
  const handleDelete = (_id) => {
    ...
    axios.delete('http://localhost:3002/api/maincategbio/' + editcatgId + '/subcategoriebio/' + _id)
    ...
    <div>
      ...
      <div className="userList">
        ...
        <div className="button">
          <Link to={"/categs1/" + editcatgId}>
            <button  className="userAddButton"></button>
          </Link>
        </div>
